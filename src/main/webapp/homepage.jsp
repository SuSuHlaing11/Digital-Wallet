...
<title>Digital Wallet by Group-6</title>
<!--     Fonts and icons     -->
<link
...
<style>
/* Container to center the stars and the button */
/* Keeps the reverse visual order */
label.star {
	float: right; /* Keeps the reverse visual order */
	padding: 6px; /* Reduced padding for smaller stars */
	font-size: 20px; /* Adjusted to make stars smaller */
	color: #fff;
	cursor: pointer;
	transition: transform 0.2s ease-in-out;
}

/* Add hover effect to increase the size of the star */
label.star:hover {
	transform: scale(1.3) rotate(-15deg);
	/* Increase size and rotate when hovered */
}

/* Add animation when the star is clicked */
input.star:checked ~ label.star, input.star:checked ~ label.star ~ label.star
	{
	color: #FD4;
	transform: scale(1.4); /* Increase size when clicked */
}
...
button[type="submit"] {
	...
	width: 100%; /* Make the button take up the full width of the box */
	margin-top: 50px; /* Add space between button and stars */
}

button[type="submit"]:hover {
	background-color: #3e0d6a; /* Darker purple when hovered */
	transform: scale(1.05); /* Slight scale-up effect when hovered */
}
...
try {
	conn = DatabaseConnection.getConnection(); // Single connection for all operations
	...
	psBalance.setInt(1, accno); // Use setString(1, String.valueOf(accno)) if accno is VARCHAR
	...
	if (transactionCounts.length() > 0)
		transactionCounts.setLength(transactionCounts.length() - 1);

	String rating = request.getParameter("rating");
	Integer Saccno = (Integer) session.getAttribute("accno");
	...
	if (rating != null) {
		...
		ps.setInt(1, accno); // Use setInt since accno is an Integer
		ps.setString(2, rating); // Rating is coming as a String from the form
		int rowsAffected = ps.executeUpdate();

		if (rowsAffected > 0) {
	System.out.println("Rating inserted successfully!");

	// Set a session attribute indicating the rating has been submitted
	session.setAttribute("ratingSubmitted", true);
		} else {
...
<body class="g-sidenav-show  profilemain" style="overflow-y: hidden;">
				...
				<ul class="navbar-nav">
					...
					<li class="nav-item"><a class="nav-link " id="hover-link"
		                ...
		                <g stroke="none" stroke-width="1" fill="none"
		                  ...
		                  <g transform="translate(-2169.000000, -745.000000)"
							...
							style="font-size: 13px;"><%=LanguageHelper.getMessage("exchange_converter", lang)%></span>
					</a></li>

					<!-- Account Pages  -->
					<li class="nav-item mt-3">
						<h6
							class="ps-4 ms-2 text-uppercase text-xs font-weight-bolder opacity-6 text-white"><%=LanguageHelper.getMessage("acc_setting", lang)%>
						</h6>
					</li>

					<!-- Account setting  -->
					<li class="nav-item"><a class="nav-link" id="hover-link"
						href="#manageUserDropdown2" data-bs-toggle="collapse"
						aria-expanded="false">
							<div
								class="icon icon-shape icon-sm shadow border-radius-md text-center me-2 d-flex align-items-center justify-content-center bg-purple">
		                ...
		                <g stroke="none" stroke-width="1" fill="none"
		                  ...
		                  <g transform="translate(-1717.000000, -291.000000)"
							...
							style="font-size: 13px;"><%=LanguageHelper.getMessage("my_acc", lang)%></span>
					</a>
						<ul id="manageUserDropdown2" class="collapse"
							style="list-style: none;">
							<li class="nav-item"><a class="nav-link" id="hover-link"
								href="profile.jsp"> <span class="nav-link-text text-white"
									style="font-size: 13px; padding-left: 35px; margin: 0;"><%=LanguageHelper.getMessage("profile", lang)%></span>
							</a></li>
							<li class="nav-item"><a class="nav-link" id="hover-link"
								href="change_password.jsp"> <span
									class="nav-link-text text-white"
									style="font-size: 13px; padding-left: 35px; margin: 0;"><%=LanguageHelper.getMessage("change_psw", lang)%></span>
							</a></li>
						</ul></li>
					...
					<li class="nav-item"><a class="nav-link " id="hover-link"
						href="logout.jsp">
							...
							style="font-size: 13px;"><%=LanguageHelper.getMessage("signout", lang)%></span>
					</a></li>
				...
				<nav aria-label="breadcrumb" style="padding-top: 0px;">
					<ol
						...
						<li class="breadcrumb-item text-sm text-white active"
							aria-current="page"><%=LanguageHelper.getMessage("home_page", lang)%></li>
					</ol>
					<h6 class="text-white font-weight-bolder ms-2"><%=LanguageHelper.getMessage("acc_no", lang)%>:
						<%=accno%></h6>
					<p class="ms-2 mb-0 text-white font-weight-bolder">
						<%=LanguageHelper.getMessage("balance", lang)%>: <span
							id="balance" style="">******</span> <i id="toggleBalance"
							class="fa fa-eye-slash ms-1"
				...
				<div class="col-lg-11">
					<div class="row">
								...
								<div class="overflow-hidden position-relative border-radius-xl"
									...
									<div class="card-body position-relative z-index-1 p-3">
										...
										<div class="d-flex">
											<div class="d-flex">
												<div class="me-4">
													<p class="text-white text-sm opacity-8 mb-0"><%=LanguageHelper.getMessage("card_holder", lang)%></p>
						...
						<div class="col-xl-6">
							<div class="row">
								<div class="col-md-6">
									<div class="card bg-glass2" style="height: 218px;">
										<div class="card-header mx-4 p-3 text-center bg-transparent">
											<div
												class="icon icon-shape icon-lg bg-purple shadow text-center border-radius-lg">
												<i class="fas fa-landmark opacity-10 homeicon"></i>
											</div>
										</div>
										<div class="card-body pt-0 p-3 text-center">
											<h6 class="text-center mb-0 text-white"><%=acctype%></h6>
											<!-- <span class="text-xs">Belong Interactive</span> -->
											<hr class="horizontal dark my-3">
											<h5 class="mb-0 text-white"><%=balance%></h5>
										</div>
									</div>
								</div>
								<div class="col-md-6 mt-md-0 mt-4">
									<div class="card bg-glass3" style="height: 218px;">
										<div class="card-body pt-0 p-3 text-center">
											...
											<div class="stars">
												<form method="post">
													<input class="star star-1" id="star-1" type="radio"
														name="rating" value="5" style="display: none;" /> <label
														class="star star-1" for="star-1">&#9733;</label> <input
														class="star star-2" id="star-2" type="radio" name="rating"
														value="4" style="display: none;" /> <label
														class="star star-2" for="star-2">&#9733;</label> <input
														class="star star-3" id="star-3" type="radio" name="rating"
														value="3" style="display: none;" /> <label
														class="star star-3" for="star-3">&#9733;</label> <input
														class="star star-4" id="star-4" type="radio" name="rating"
														value="2" style="display: none;" /> <label
														class="star star-4" for="star-4">&#9733;</label> <input
														class="star star-5" id="star-5" type="radio" name="rating"
														value="1" style="display: none;" /> <label
														class="star star-5" for="star-5">&#9733;</label>

													<hr class="horizontal dark my-3">
													<button type="submit" id="submitBtn" class="bg-glass3">
														<%
														if (session.getAttribute("ratingSubmitted") != null && (boolean) session.getAttribute("ratingSubmitted")) {
														%>
														<%=LanguageHelper.getMessage("rating2", lang)%>
														<%
														} else {
														%>
														<%=LanguageHelper.getMessage("rating1", lang)%>
														<%
														}
														%>
													</button>
						...
						<div class="col-md-12 mb-lg-0 mt-4">
							<div class="row">
								<div class="col-md-4">
									<div class="card bg-glass2">
										<div class="card-header mx-4 p-3 text-center bg-transparent">
											<div
												class="icon icon-shape icon-lg bg-purple shadow text-center border-radius-lg">
												<a href="./deposit.jsp"> <svg width="30px" height="70px"
										...
										<div class="card-body pt-0 p-3 text-center">
											<a href="./deposit.jsp">
												<h6 class="text-center mb-0 text-white"><%=LanguageHelper.getMessage("deposit", lang)%></h6>
											</a>
										</div>
									</div>
								</div>
								<div class="col-md-4 mt-md-0 mt-4">
									<div class="card bg-glass3">
										<div class="card-header mx-4 p-3 text-center bg-transparent">
											<div
												class="icon icon-shape icon-lg bg-purple shadow text-center border-radius-lg">
												<a href="./withdraw.jsp"> <svg width="30px"
														height="60px" viewBox="0 0 16 16" version="1.1"
														xmlns="http://www.w3.org/2000/svg"
														xmlns:xlink="http://www.w3.org/1999/xlink" fill="#000000">
													<g id="SVGRepo_bgCarrier" stroke-width="0" />
													<g id="SVGRepo_tracerCarrier" stroke-linecap="round"
										...
										<div class="card-body pt-0 p-3 text-center">
											<a href="./withdraw.jsp">
												<h6 class="text-center mb-0 text-white"><%=LanguageHelper.getMessage("withdraw", lang)%></h6>
											</a>
										</div>
									</div>
								</div>
								<div class="col-md-4 mt-md-0 mt-4">
									<div class="card bg-glass2">
										<div class="card-header mx-4 p-3 text-center bg-transparent">
											<div
												class="icon icon-shape icon-lg bg-purple shadow text-center border-radius-lg">
												<a href="./transfer.jsp"> <svg fill="#000000"
														width="50px" height="65px" viewBox="-2.5 0 19 19"
														xmlns="http://www.w3.org/2000/svg" class="cf-icon-svg">
                            						<path
															d="M13.014 7.026v5.142a.476.476 0 0 1-.475.475H1.461a.476.476 0 0 1-.475-.475V7.026a.476.476 0 0 1 .475-.475h11.078a.476.476 0 0 1 .475.475zm-3.582 2.57a2.424 2.424 0 1 0-.191.947 2.417 2.417 0 0 0 .19-.946zm-.235-4.581a.554.554 0 0 1-.783 0l-.86-.86v1a.554.554 0 1 1-1.108 0v-1l-.86.86a.554.554 0 0 1-.783-.784l1.805-1.805a.554.554 0 0 1 .784 0L9.197 4.23a.554.554 0 0 1 0 .784zm0 9.947-1.805 1.806a.554.554 0 0 1-.784 0l-1.805-1.806a.554.554 0 0 1 .783-.783l.86.86v-1a.554.554 0 0 1 1.108 0v1l.86-.86a.554.554 0 1 1 .783.783zm-1.05-4.729a.83.83 0 0 1-.416.716 1.326 1.326 0 0 1-.413.173v.266a.317.317 0 0 1-.633 0v-.262a1.449 1.449 0 0 1-.268-.084.943.943 0 0 1-.362-.265.317.317 0 0 1 .479-.415.32.32 0 0 0 .118.092.842.842 0 0 0 .162.052 1.248 1.248 0 0 0 .176.022.765.765 0 0 0 .397-.11c.127-.082.127-.152.127-.185a.218.218 0 0 0-.053-.14.504.504 0 0 0-.132-.106.668.668 0 0 0-.163-.058.765.765 0 0 0-.16-.017 1.642 1.642 0 0 1-.273-.021 1.248 1.248 0 0 1-.352-.114 1.018 1.018 0 0 1-.335-.269.864.864 0 0 1-.198-.542.876.876 0 0 1 .435-.74 1.314 1.314 0 0 1 .402-.165v-.255a.317.317 0 0 1 .633 0v.264a1.513 1.513 0 0 1 .273.095 1.085 1.085 0 0 1 .318.218.317.317 0 0 1-.448.449.454.454 0 0 0-.13-.09.887.887 0 0 0-.167-.058l-.017-.004a.842.842 0 0 0-.137-.023.768.768 0 0 0-.389.104.249.249 0 0 0-.14.205.242.242 0 0 0 .057.143.394.394 0 0 0 .126.101.627.627 0 0 0 .173.056 1.01 1.01 0 0 0 .169.013 1.4 1.4 0 0 1 .295.03 1.305 1.305 0 0 1 .323.117 1.13 1.13 0 0 1 .314.25.848.848 0 0 1 .21.557z" />
                          						</svg>
										...
										<div class="card-body pt-0 p-3 text-center">
											<a href="./transfer.jsp">
												<h6 class="text-center mb-0 text-white"><%=LanguageHelper.getMessage("transfer", lang)%></h6>
											</a>
										</div>
									</div>
								</div>
	...
	<div class="modal fade" id="frozenModal" tabindex="-1"
		aria-labelledby="frozenModalLabel" aria-hidden="true">
			...
			<div class="modal-content">
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-primary"
						data-bs-dismiss="modal"><%=LanguageHelper.getMessage("ok", lang)%></button>
				</div>
			</div>
		</div>
	</div>
