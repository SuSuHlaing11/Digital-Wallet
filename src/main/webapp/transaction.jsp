...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <div class="collapse navbar-collapse  w-auto vh-100" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active " href="transaction.jsp">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
...
<%@ page import="java.sql.*, com.bank.database.DatabaseConnection, java.util.Set, java.util.HashSet"%>
<%
    ...
    Set<String> deletedUserAcc = new HashSet<>();
    ...
    try {
        ...
        String deletedUser = "SELECT accno FROM user WHERE is_deleted = 1";
        ps1 = conn.prepareStatement(deletedUser);
        rs1 = ps1.executeQuery();

        while (rs1.next()) {
            deletedUserAcc.add(rs1.getString("accno"));
        }
        ps1.close();
        rs1.close();

        // Query to fetch transaction history, with dynamic sorting based on user input
        String sql = "SELECT * FROM history ORDER BY transfer_time " + sortOrder;
...
<script>
    document.getElementById('exportBtn').addEventListener('click', function() {
        ...
        if (exportType === 'pdf') {
            ...
            doc.text("Transaction Table Export", 14, 10);
        ...
        <tbody>
          <% while (rs.next()) { 
            String senderId = rs.getString("sender_id");
              ...
              if (deletedUserAcc.contains(senderId)) {
                  senderDeleted = true;
              }
  
              if (deletedUserAcc.contains(receiverId)) {
          ...
          <tr style="vertical-align: middle;">
              ...
              <td>
                  <% if (senderDeleted) { %>
                          ...
                          <span style="color: red;"><%= senderId %></span>
                      <% } else { %>
                          <%= senderId %>
                      <% } %>
              </td>
              ...
              <td>
                  <% if (receiverDeleted) { %>
                      <span class="badge bg-danger">Deleted</span>
                      <span style="color: red;"><%= receiverId %></span>
                  <% } else { %>
                      <%= receiverId %>
                  <% } %>
              </td>
                  ...
                  <td><%= rs.getString("note") != null ? rs.getString("note") : "N/A" %></td>
                   <td>
              <button class="alert alert-danger text-center " style="border-radius: 5px; color: white; margin-bottom:0px !important;" data-transfer-id="<%= rs.getInt("transfer_id") %>">
                  Send Alert
              </button>
          </td>
