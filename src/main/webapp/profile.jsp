...
%>
  <!-- Only includes language switcher, not full profile UI -->
<%@ page session="true"%>
<%
...
String profilePicture = "img/No_Img.jpg";

// Fetch profile picture from the session
String dbProfilePicture = (String) session.getAttribute("profile_picture");
if (dbProfilePicture != null && !dbProfilePicture.isEmpty()) {
	profilePicture = dbProfilePicture;
}

Connection conn = null;
PreparedStatement psUser = null, psBalance = null;
ResultSet rsUser = null, rsBalance = null;
...
try {
	...
	"SELECT fullname, username, dob, gender, nrc, address, email, phone, acctype, profile_picture FROM user WHERE accno = ?");
	...
	if (rsUser.next()) {
		...
		nrc = rsUser.getString("nrc");
		address = rsUser.getString("address");
		email = rsUser.getString("email");
		phone = rsUser.getString("phone");
		acctype = rsUser.getString("acctype");

		// Fetch profile picture from DB if available
		String tempProfilePic = rsUser.getString("profile_picture");
...
// Force image refresh
profilePicture += "?t=" + System.currentTimeMillis();
%>

<body class="g-sidenav-show  profilemain">
	<aside
			...
			<div class="collapse navbar-collapse  w-auto vh-100"
				...
				<ul class="navbar-nav">
					...
					<li class="nav-item"><a class="nav-link active"
						id="hover-link" href="#manageUserDropdown2"
						...
						<ul id="manageUserDropdown2" class="collapse"
							...
							<li class="nav-item"><a class="nav-link" id="hover-text"
	...
	<main
			...
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
						<h5 class="modal-title" id="successModalLabel"><%=LanguageHelper.getMessage("profile_submit", lang)%></h5>
						<button type="button" class="btn-close" data-bs-dismiss="modal"
							aria-label="Close"></button>
					</div>
					<div class="modal-body"><%=LanguageHelper.getMessage("profile_submit1", lang)%></div>
		...
		<!-- Trigger Modal If Profile Updated -->
		<script>
			document
					.addEventListener(
							"DOMContentLoaded",
							function() {
								var profileUpdated =
		<%=session.getAttribute("profileUpdateSuccess") != null ? "true" : "false"%>
			;
				...
				<nav aria-label="breadcrumb">
					<ol
						...
						<li class="breadcrumb-item text-sm"><a
							class="opacity-5 text-white" href="javascript:;"><%=LanguageHelper.getMessage("user", lang)%></a></li>
						<li class="breadcrumb-item text-sm text-white active"
							aria-current="page"><%=LanguageHelper.getMessage("profile1", lang)%></li>
					</ol>
					<h6 class="font-weight-bolder mb-0 text-white"><%=LanguageHelper.getMessage("profile", lang)%></h6>
				</nav>
				...
				<div class="collapse navbar-collapse me-md-0 me-sm-4 mt-sm-0 mt-2"
					...
					<ul class="navbar-nav justify-content-end">
					    <li class="nav-item dropdown">
					        <a class="nav-link d-flex align-items-center me-2 active dropdown-toggle text-white"
					           href="#" id="languageDropdown" role="button" data-bs-toggle="dropdown"
					           aria-expanded="false" style="font-size: 13px;"> 
					            <i class="fas fa-language me-1"></i>
					            <%= LanguageHelper.getMessage("language", lang) %>
					        </a>
					        <ul class="dropdown-menu" aria-labelledby="languageDropdown">
					            <li><a class="dropdown-item" href="ChangeLanguageServlet?lang=en">English</a></li>
					            <li><a class="dropdown-item" href="ChangeLanguageServlet?lang=mm">မြန်မာ</a></li>
					        </ul>
					    </li>
					    
					    <li class="nav-item d-flex align-items-center ms-3"> 
					        <a href="logout.jsp" id="hover-link" class="nav-link text-white font-weight-bold px-0" style="font-size: 13px;">
					            <i class="fa fa-user me-sm-1"></i> 
					            <span class="d-sm-inline d-none text-white">
					                <%= LanguageHelper.getMessage("sign_out", lang) %>
					            </span>
					        </a>
					    </li>
					</ul>
		...
		<div class="container-fluid ">
			<div class="bg-glass2">
				<div
					class="page-header min-height-80 border-radius-lg mt-2 d-flex flex-column justify-content-end">
						...
						<div class="d-flex justify-content-between align-items-end">
							<div class="d-flex align-items-center" style="margin-left: 10px;">
								...
								<div>
									...
									<p class="mb-0 text-white font-weight-bolder">
								...
								<div class="d-flex justify-content-end mt-4"
									...
									<a href="editprofile.jsp">
										<button type="submit"
											class="btn bg-purple text-center text-white m-0 ms-2"
											style="width: 120px;"><%=LanguageHelper.getMessage("edit_profile", lang)%></button>
									</a>
								</div>
						...
						<div class="card-body pb-0 p-3">
							...
							<div class="row">
								<div class="col-md-6">
									...
									<h4 class="text-white bg-transparent"><%=LanguageHelper.getMessage("personal_info", lang)%></h4>
									<hr class=" horizontal dark my-1">
									<ul class="list-group mb-3">
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white"><strong>
												<%=LanguageHelper.getMessage("acc_no", lang)%>:</strong> &nbsp; <%=accno%></li>
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white"><strong>
												<%=LanguageHelper.getMessage("acc_type", lang)%>:</strong> &nbsp; <%=acctype%></li>
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white"><strong>
												<%=LanguageHelper.getMessage("fullname", lang)%>:</strong> &nbsp; <%=fullname%></li>
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white">
											<strong><%=LanguageHelper.getMessage("username", lang)%>:</strong> &nbsp; <%=username%>
										</li>
										
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white"><strong>
											<%=LanguageHelper.getMessage("dob", lang)%>:</strong> &nbsp; <%=dob%></li>
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white"><strong>
											<%=LanguageHelper.getMessage("gender", lang)%>:</strong>
											&nbsp; <%=gender%></li>
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white"><strong>
											<%=LanguageHelper.getMessage("nrc", lang)%>:</strong>
											&nbsp; <%=nrc%></li>
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white">
											<strong><%=LanguageHelper.getMessage("location", lang)%>:</strong> &nbsp; <%=address%>
										</li>
										
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white">
											<strong><%=LanguageHelper.getMessage("email", lang)%>:</strong> &nbsp; <%=email%>
										</li>
										
										<li
											class="bg-transparent list-group-item border-0 ps-0 text-sm text-white">
											<strong><%=LanguageHelper.getMessage("mobile", lang)%>:</strong> &nbsp; <%=phone%>
										</li>
	...
	<script>
		document.addEventListener("DOMContentLoaded", function() {
			...
			var actualBalance = balanceElement
					.getAttribute("data-actual-balance"); // Get actual balance

			toggleBalance.addEventListener("click", function() {
				console.log("Toggle clicked!"); // Debugging log

				if (balanceElement.textContent.trim() === "******") {
					balanceElement.textContent = actualBalance; // Show balance
