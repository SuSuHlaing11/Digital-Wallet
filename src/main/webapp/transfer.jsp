			...
			<div class="container-fluid py-1 px-3">
				<nav aria-label="breadcrumb" style="padding-top:0px;">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 ps-2 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-white active" aria-current="page"><%=LanguageHelper.getMessage("transfer", lang)%></li>
                    </ol>
                    ...
                    <div class="card my-4 bg-glass3">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2 bg-transparent">
                            <div class="bg-purple shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-capitalize ps-3"><%=LanguageHelper.getMessage("transfer_funds", lang)%></h6>
                            </div>
                        </div>
                        <div class="card-body px-4 pb-2">
                            	<div class="mb-3">
                                    <label for="cardNumber" class="form-label text-white"><%=LanguageHelper.getMessage("accno1", lang)%></label>
...
<%
						...
						%>
							...
							%>                 
<form 
action="${pageContext.request.contextPath}/TransferController"
 method="post" onsubmit="return validateTransfer()">
    <div class="mb-3">
        <label for="amount" class="form-label text-white"><%=LanguageHelper.getMessage("amt", lang)%></label>
        <div class="input-group input-group-outline">
            <input type="number" class="form-control transparent-input" id="amount" name="amount" placeholder="<%=LanguageHelper.getMessage("enter_amt", lang)%>" step="0.01" required>
        </div>
    </div>

    <div class="mb-3">
        <label for="cardNumber" class="form-label text-white"><%=LanguageHelper.getMessage("accno2", lang)%></label>
        <div class="input-group input-group-outline">
            <input type="text" class="form-control transparent-input" id="cardNumber" name="toAccount" placeholder="000000" required>
        </div>
    </div>
    ...
    <div class="mb-3">
        ...
        <div class="input-group input-group-outline">
            <input type="password" class="form-control transparent-input" id="password" name="password" placeholder="*** *** *****" required>
        </div>
    </div>

    <div class="text-center">
        <button type="submit" class="btn btn-lg bg-purple btn-lg w-100 mt-4 mb-0 text-white">
    ...
    function validateTransfer() {
        ...
        // Example: Maximum transfer limit of 10,000,000
        if (amount > 10000000) {
            amountError.textContent = "Maximum transfer limit is 10,000,000.";
            return false;
        } else {
        ...
        if (amount <= 0) {
            amountError.textContent = "Transfer amount must be a positive number.";
            return false;
        }
...
<script>
    // Show success message alert when transfer is confirmed
    document.getElementById("confirmButton").addEventListener("click", function() {
        // Display transfer success alert
        alert("Your transfer has been successfully completed!");
    });
</script>
        <!-- Github buttons -->
