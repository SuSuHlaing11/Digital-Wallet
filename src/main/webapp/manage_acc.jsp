...
<%
    if (request.getAttribute("userList") == null) {
        response.sendRedirect(request.getContextPath() + "/manage-accounts");
        return;
    }
%>
<!DOCTYPE html>
...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <div class="collapse navbar-collapse  w-auto vh-100" id="sidenav-collapse-main">
      <ul class="navbar-nav">
        ...
        <li class="nav-item">
          <a class="nav-link active" href="manage_acc.jsp">
            <div class="icon icon-shape icon-sm shadow border-radius-md bg-white text-center me-2 d-flex align-items-center justify-content-center">
    ...
    <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl" id="navbarBlur" navbar-scroll="true">
        <div class="container-fluid py-1 px-3">
          <nav aria-label="breadcrumb">
            ...
            <h6 class="font-weight-bolder mb-0">Manage Accounts</h6>
          </nav>
                            ...
                            <table id="example23" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                ...
                                <tbody>
                                     <c:forEach items="${userList}" var="user">
					                    <tr>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.id}</td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.accno} 
										        <c:if test="${user.deleted}">
										            <span class="badge bg-danger">Deleted</span>
										        </c:if>
										    </td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.fullname}</td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.username}</td>
					                        <td style="${user.deleted ? 'color: red;' : ''}"><fmt:formatDate value="${user.dob}" pattern="dd-MM-yyyy" /></td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.gender}</td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.nrc}</td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.email}</td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">0${user.phone}</td>
					                        <td style="${user.deleted ? 'color: red;' : ''}">${user.acctype}</td>
					                        <td>
					                        	<c:if test="${not user.deleted}">
												    <button class="btn btn-sm ${user.frozen ? 'btn-success' : 'btn-danger'} freeze-btn" 
												            data-id="${user.id}" 
												            data-action="${user.frozen ? 'unfreeze' : 'freeze'}">
												        <i class="fas ${user.frozen ? 'fa-unlock' : 'fa-lock'}"></i>
          ...
          <div class="row align-items-center justify-content-lg-between">
            <div class="col-lg-6 mb-lg-0 mb-4">
              <div class="copyright text-center text-sm text-muted text-lg-start">
                ...
                made<i class="fa fa-money"></i> by Digital Wallet Team for a better web.
              </div>
            </div>
            <div class="col-lg-6">
...
<!--   Core JS Files   -->
  ...
  <script>
	    ...
	    $(".freeze-btn").click(function() {
	            ...
	            success: function(response) {
	                if (response.success) {
	                    if (action === "freeze") {
	                        ...
	                        button.html('<i class="fas fa-unlock"></i> Unfreeze');
	                        button.data("action", "unfreeze");
	                    } else {
	                    	updateSessionStatus();
	                        button.removeClass("btn-success").addClass("btn-danger");
	                        button.html('<i class="fas fa-lock"></i> Freeze');
	  	...
	  	function updateSessionStatus() {
	  	    fetch("/getSessionData")
	  	        ...
	  	        .then(data => {
	  	            sessionStorage.setItem("accountFrozen", data.accountFrozen);
	  	            console.log("Updated Account Frozen Status:", data.accountFrozen);
	  	        });
	  	}
  	...
  	$(document).on("click", ".delete-btn", function() {
  	    ...
  	    var $confirmBtn = $('#confirmDelete');
  	    var $okBtn = $('#deleteModal .btn-primary');
  	    var $cancelBtn = $('#deleteModal .btn-secondary');

  	    $modalBody.html("Are you sure you want to delete this user? This action cannot be undone.");
  	    $confirmBtn.show().off('click');
  	    $cancelBtn.show();
  	    $okBtn.hide();
  	    deleteModal.show();
  	    ...
  	    $confirmBtn.on('click', function() {
  	        ...
  	        $modalBody.html(`
  	            <div class="d-flex align-items-center">
  	                ...
  	                Deleting account and sending notification...
  	            </div>
  	        `);
  	                ...
  	                if (response.success) {
  	                    /*$modalBody.html(`
  	                        <div class="alert alert-success">
  	                            ...
  	                            <small class="text-muted">Confirmation email sent to user.</small>
  	                        </div>
  	                    `);*/
  	                  	$modalBody.html("Account deleted successfully! Confirmation email sent to user.");
  	                    ...
  	                    showError(response.error || "Error deleting user");
  	                }
  	            },
  	            error: function(xhr) {
  	                showError(xhr.statusText || "Connection error");
  	            }
  	        });
