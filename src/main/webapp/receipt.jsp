...
try {
	...
	if (rsUser.next()) {
		...
		String dbProfilePicture = rsUser.getString("profile_picture");
		if (dbProfilePicture != null && !dbProfilePicture.isEmpty()) {
	profilePicture = dbProfilePicture; // Use stored image
		}
	}
		...
		<div class="bg-glass2">
			...
			<div class="collapse navbar-collapse  w-auto vh-100"
				...
				<ul class="navbar-nav">
					...
					<!-- Account setting  -->
					<li class="nav-item"><a class="nav-link " id="hover-link" href="#manageUserDropdown2"
		...
		<nav
			...
			<div class="container-fluid py-1 px-3">
				<nav aria-label="breadcrumb">
					...
					<h6 class="font-weight-bolder mb-0 text-white"><%=LanguageHelper.getMessage("receipt", lang)%></h6>
				</nav>
...
<div class="container-fluid py-4">
    <div class="row d-flex justify-content-center">
        <div class="col-12 col-xl-9">
            <div class="card h-100 bg-glass3" id="receipt">
                <div class="card-body pb-0 p-3">
<div class="row">
    <div class="col-6 d-flex align-items-center">
        <a class="navbar-brand m-0" href="index.jsp" target="_blank">
            <img src="img/vue.png" style="height:40px; width:auto;" class="navbar-brand-img h-10" alt="main_logo">
            <span class="ms-1 font-weight-bold"><%=LanguageHelper.getMessage("title", lang)%></span>
        </a>
    </div>
    <div class="col-6 d-flex justify-content-end align-items-center text-white">
		...
		<div class="text-center mt-4">
		    <i class="fa-solid fa-check-circle text-success fa-3x"></i> <!-- Success Icon -->
		    <h3 class="text-white mt-2"><%=LanguageHelper.getMessage("trans_success", lang)%></h3>
		    <h3 class="text-white fw-bold">-${Tamount} <%=LanguageHelper.getMessage("ks", lang)%></h3><br> <!-- Display Amount -->
		</div>
		<hr class="horizontal dark my-1">
		
		<h4 class="text-center text-white bg-transparent mt-4"><%=LanguageHelper.getMessage("receipt", lang)%></h4>
		<div class="row">
				    <div class="col-md-12 col-sm-12 mt-3">
				        <ul class="list-group mb-3">
				            <li class="bg-transparent list-group-item border-0 text-sm text-white d-flex justify-content-between">
				                <strong><%=LanguageHelper.getMessage("receipt_id", lang)%>:</strong> <span class="text-end flex-grow-1">${transferId}</span>
				            </li>
				            
				            <li class="bg-transparent list-group-item border-0 text-sm text-white d-flex justify-content-between">
				                <strong><%=LanguageHelper.getMessage("trans_type", lang)%>:</strong> <span class="text-end flex-grow-1">${transactionType}</span>
				            </li>
				            
				            <li class="bg-transparent list-group-item border-0 text-sm text-white d-flex justify-content-between">
				                <strong><%=LanguageHelper.getMessage("my_acc", lang)%>:</strong> <span class="text-end flex-grow-1">${accno}</span>
				            </li>
				            
				            <% if ("Transfer".equals(request.getAttribute("transactionType"))) { %>
				            <li class="bg-transparent list-group-item border-0 text-sm text-white d-flex justify-content-between">
				                <strong><%=LanguageHelper.getMessage("receiver_acc", lang)%>:</strong> <span class="text-end flex-grow-1">${receiptId}</span>
				            </li>
				            <% } %>
				            
				            <li class="bg-transparent list-group-item border-0 text-sm text-white d-flex justify-content-between">
				                <strong><%=LanguageHelper.getMessage("amt", lang)%>:</strong> <span class="text-end flex-grow-1">-${Tamount} Ks</span>
				            </li>
				            
				            <li class="bg-transparent list-group-item border-0 text-sm text-white d-flex justify-content-between">
				                <strong><%=LanguageHelper.getMessage("date_time", lang)%>:</strong> <span class="text-end flex-grow-1">${transactionDate}</span>
				            </li>
				...
				<!-- "Thank You" Message -->
				<div class="text-center mt-2" style="border-top: 2px dotted white; margin-top: 40px;">
				    <h5 class="text-white" style="margin-top: 40px;"><%=LanguageHelper.getMessage("receipt_success", lang)%></h5>
				</div>
                    
                    <hr>
                    <div class="d-flex justify-content-center">
					    <button class="btn mt-3 bg-purple text-white" id="saveReceipt"><%=LanguageHelper.getMessage("save_img", lang)%></button>
					</div>
				</div>
            </div>
        </div>
...
confetti({
    particleCount: 100,
    spread: 70,
    origin: { x: 0.6, y: 0.5 } // Center of the screen
});
...
<script>
    document.getElementById("saveReceipt").addEventListener("click", function () {
        ...
        // Temporarily hide the button but keep the space
        ...
        html2canvas(receipt, {
            ...
            // Create a temporary link to download image
            ...
            button.style.visibility = "visible"; // Ensure button is always restored
        });
    });
</script>
    </main>
    ...
    document.getElementById("toggleBalance").addEventListener("click", function () {
        ...
        if (balanceElement.innerText === "******") {
            ...
            balanceElement.innerText="******"
							; // Hidebalancethis.classList.remove("fa-eye");
