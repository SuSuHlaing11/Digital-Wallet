...
<%
    ...
    if (request.getMethod().equalsIgnoreCase("POST")) {
        ...
        try {
            ...
            byte[] oldHashedBytes = md.digest();
            StringBuilder sbOld = new StringBuilder();
            for (byte b : oldHashedBytes) {
                sbOld.append(String.format("%02x", b));
            }
            String oldHashedPwd = sbOld.toString();

            // Check if the old password is correct
            String checkUserQuery = "SELECT password FROM user WHERE accno = ?";
            stmt = conn2.prepareStatement(checkUserQuery);
            stmt.setInt(1, accno);
            ...
            if (rs.next()) {
                ...
                if (!storedPassword.equals(oldHashedPwd)) {
                    errorMessage = LanguageHelper.getMessage("old_notcor", lang);
                } else if (!newPassword.equals(confirmPassword)) {
                    errorMessage = LanguageHelper.getMessage("not_match", lang);
                } else {
                    // Hash the new password using MD5
                    MessageDigest mdNew = MessageDigest.getInstance("SHA-256");
                    mdNew.update(newPassword.getBytes());
                    byte[] newHashedBytes = mdNew.digest();
                    StringBuilder sbNew = new StringBuilder();
                    for (byte b : newHashedBytes) {
                        sbNew.append(String.format("%02x", b));
                    }
                    String newHashedPwd = sbNew.toString();

                    // Update password
                    String updateQuery = "UPDATE user SET password = ? WHERE accno = ?";
                    updateStmt = conn2.prepareStatement(updateQuery);
                    updateStmt.setString(1, newHashedPwd);
                    updateStmt.setInt(2, accno);
                    int rowsAffected = updateStmt.executeUpdate();

                    if (rowsAffected > 0) {
                        successMessage = LanguageHelper.getMessage("psw_upd", lang);
                    } else {
                        errorMessage = LanguageHelper.getMessage("psw_not_upd", lang);
                    }
                }
            } else {
                errorMessage = LanguageHelper.getMessage("acc_not_found", lang);
            }
		          ...
		          <a class="nav-link active" href="#manageUserDropdown2" data-bs-toggle="collapse" aria-expanded="false">
		            ...
		            <span class="nav-link-text ms-1 text-white" style="font-size:13px;" ><%=LanguageHelper.getMessage("my_acc", lang)%></span>
		          		...
		          		<ul id="manageUserDropdown2" class="collapse" style="list-style: none;">
		                    ...
		                    <li class="nav-item">
		                    	<a class="nav-link" id="hover-text" href="change_password.jsp">
			...
			<div class="container-fluid py-1 px-3">
				<nav aria-label="breadcrumb" style="padding-top:0px;">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 ps-2 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-white active" aria-current="page"><%=LanguageHelper.getMessage("change_psw", lang)%></li>
                    </ol>
				...
				<div class="collapse navbar-collapse me-md-0 me-sm-4 mt-sm-0 mt-2" id="navbar">
                   ...
                   <ul class="navbar-nav  justify-content-end">
					    <li class="nav-item d-flex align-items-center">
					      <a href="logout.jsp" id="hover-link" class="nav-link text-white font-weight-bold px-0">
					        <i class="fa fa-user me-sm-1"></i>
					        <span class="d-sm-inline d-none text-white" style="font-size:13px;"><%=LanguageHelper.getMessage("sign_out", lang)%></span>
					      </a>
					    </li>
					</ul>
                    ...
                    <div class="card my-4 bg-glass3">
                        <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2 bg-transparent">
                            <div class="bg-purple shadow-primary border-radius-lg pt-4 pb-3">
                                <h6 class="text-white text-center text-capitalize ps-3"><%=LanguageHelper.getMessage("change_psw", lang)%></h6>
                            </div>
                        </div>
                        <div class="card-body px-4 pb-2">
                             <form method="post" onsubmit="return validatePassword()">
						        ...
						        <% } %>
                            	<div class="mb-3">
                                    <label for="cardNumber" class="form-label text-white"><%=LanguageHelper.getMessage("acc_no", lang)%></label>
                                    <div class="input-group input-group-outline">
                                        <input type="text" class="form-control transparent-input" id="cardNumber"
    										value="<%= accno %>" disabled style="color: black !important;">  
    								</div>
                                </div>
                                
                                <div class="mb-3">
                                    <label for="dateTime" class="form-label text-white"><%=LanguageHelper.getMessage("old_psw", lang)%></label>
                                    <div class="input-group input-group-outline">
                                        <input type="password" name="old_password" id="old_password" class="form-control transparent-input" 
                                            placeholder="*** *** *****" required>
                                    </div>
                                </div>
                                
                                <div class="mb-3">
                                    <label for="dateTime" class="form-label text-white"><%=LanguageHelper.getMessage("new_psw", lang)%></label>
                                    <div class="input-group input-group-outline">
                                        <input type="password" name="new_password" id="new_password" class="form-control transparent-input"
                                            placeholder="*** *** *****" required>
                                    </div>
                                </div>
                                
                                <div class="mb-3">
                                    <label for="dateTime" class="form-label text-white"><%=LanguageHelper.getMessage("c_password", lang)%></label>
                                    <div class="input-group input-group-outline">
                                        <input type="password" name="confirm_password" id="confirm_password" class="form-control transparent-input" 
                                            placeholder="*** *** *****" required>
                                    </div>
                                </div>

                                <div class="text-center"><span id="confirmMessage"></span>
                                    <button type="submit" class="btn btn-lg bg-purple btn-lg w-100 mt-4 mb-0 text-white" id="confirmButton">
                                        <%=LanguageHelper.getMessage("confirm", lang)%>
                                    </button>
                                </div>
                            </form>
                        </div>
...
<div class="modal fade" id="emailModal" tabindex="-1" aria-labelledby="emailModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="emailModalLabel">Email Sent!</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        Go check your email for your account number!
      </div>
      <div class="modal-footer">
