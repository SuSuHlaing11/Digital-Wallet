...
<%
    ...
    String requiredFieldMsg = LanguageHelper.getMessage("error.field_required", lang);
%>
<%
	...
	<div class="main-content position-relative max-height-vh-100 h-100">
				...
				<nav aria-label="breadcrumb" style="padding-top: 0px;">
					<ol
						...
						<li class="breadcrumb-item text-sm text-white active"
							aria-current="page"><%=LanguageHelper.getMessage("withdraw", lang)%></li>
					</ol>

					<h6 class="text-white font-weight-bolder ms-2"><%=fullname%></h6>

					<p class="ms-2 mb-0 text-white font-weight-bolder">
						<%=LanguageHelper.getMessage("balance", lang)%>: <span id="balance">******</span> <i id="toggleBalance"
							class="fa fa-eye-slash ms-1"
							style="font-size: 10px; cursor: pointer;"></i>
					</p>

					<!-- Hidden Input to Store Balance for JavaScript -->
					<input type="hidden" id="hiddenBalance" value="<%=balance%>">
				</nav>
					...
					<div class="card my-4 bg-glass3">
						<div
							...
							<div class="bg-purple shadow-primary border-radius-lg pt-4 pb-3">
								<h6 class="text-white text-center text-capitalize ps-3"><%=LanguageHelper.getMessage("withdraw_funds", lang)%></h6>
							</div>
						</div>
						<div class="card-body px-4 pb-2">
							...
							if (errorMessage == null) {
								errorMessage = request.getParameter("error"); // Check query parameters as a fallback
							}
							%>
							<%
							String successMessage = (String) request.getAttribute("success");
							if (errorMessage == null) {
								errorMessage = request.getParameter("success"); // Check query parameters as a fallback
							}
							%>
							...
							<!-- Withdraw Form -->
							<form
								action="${pageContext.request.contextPath}/WithdrawController"
								method="post" onsubmit="return validateWithdrawal()">
								<div class="mb-3">
									...
									<div class="input-group input-group-outline">
										<input type="text" class="form-control transparent-input"
											id="cardNumber" name="cardNumber" value="<%=accno%>" disabled
								...
								<div class="mb-3">
									...
									<div class="input-group input-group-outline">
										<input type="digit" class="form-control transparent-input"
											id="amount" name="amount" placeholder="<%=LanguageHelper.getMessage("enter_amt", lang)%>"
											step="0.01" min="0.01" max="5000000" required>
									</div>
									<small id="amountError" class="text-danger"></small>
								</div>

								<div class="mb-3">
									<label for="password" class="form-label text-white"><%=LanguageHelper.getMessage("password", lang)%></label>
									<div class="input-group input-group-outline">
								...
								<div class="text-center">
									<button type="submit"
										class="btn btn-lg bg-purple w-100 mt-4 mb-0 text-white"><%=LanguageHelper.getMessage("confirm", lang)%></button>
								</div>
							</form>
						</div>
					</div>
		...
		<div class="modal fade" id="frozenModal" tabindex="-1"
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body"><%=LanguageHelper.getMessage("frozen_msg", lang)%></div>
		...
		<script>
			document
					.addEventListener(
							"DOMContentLoaded",
							function() {
								let balanceElement = document
										.getElementById("balance");
								let hiddenBalance = document
										.getElementById("hiddenBalance").value;
										...
										.addEventListener(
												...
												function() {
													if (balanceElement.innerText === "******") {
														balanceElement.innerText = hiddenBalance; // Show balance
														this.classList
																.remove("fa-eye-slash");
														this.classList
																.add("fa-eye");
								...
								console.log("DEBUG: Hidden balance value = "
										+ hiddenBalance);
							});
		</script>
		...
		<script>
			document.addEventListener("DOMContentLoaded", function() {
				let acctypeInput = document
						.querySelector("input[name='acctype']");
				console.log(
						"DEBUG: Hidden Account Type Value Before Submit = ",
						acctypeInput.value);
			});
			...
			document.querySelector("form").addEventListener(
					...
					function() {
						let acctypeInput = document
								.querySelector("input[name='acctype']");
						console.log("DEBUG: Sending Account Type = ",
								acctypeInput.value);
					});
		</script>

		<!-- Validation Script for Withdrawal -->
