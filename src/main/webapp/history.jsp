...
<%@ page import="com.bank.database.DatabaseConnection" %>
...
<%
    ...
    String accno = session.getAttribute("accno").toString(); // Get account number from session
    ...
    try {
        ...
        // Correct SQL query to filter by sender_id or receiver_id
        String sql = "SELECT * FROM history WHERE sender_id = ? OR receiver_id = ? ORDER BY transfer_time " + sortOrder;
        ps = conn.prepareStatement(sql);
        ps.setString(1, accno); // Set account number for sender_id
        ps.setString(2, accno); // Set account number for receiver_id
        rs = ps.executeQuery();
    } catch (ClassNotFoundException e) {
        errorMessage = "MySQL JDBC Driver not found!";
    ...
    try {
        // Prepare query for calculating income and outcome
        String summarySql = "SELECT amount, sender_id, receiver_id FROM history WHERE (sender_id = ? OR receiver_id = ?) AND MONTH(transfer_time) = MONTH(CURRENT_DATE()) AND YEAR(transfer_time) = YEAR(CURRENT_DATE())";
        PreparedStatement psSummary = conn.prepareStatement(summarySql);
        psSummary.setString(1, accno);
        psSummary.setString(2, accno);
        ResultSet rsSummary = psSummary.executeQuery();

        while (rsSummary.next()) {
            BigDecimal amount = rsSummary.getBigDecimal("amount");

            if (accno.equals(rsSummary.getString("sender_id"))) {
                // If the user is the sender, it's an outcome
                totalOutcome = totalOutcome.add(amount);
            } else if (accno.equals(rsSummary.getString("receiver_id"))) {
        ...
        errorMessage = "Error calculating monthly summary: " + e.getMessage();
    }

    // Initialize account type variable
    String acctype = "Unknown";

    try {
        // Query to fetch account type from user table
        String userSql = "SELECT acctype FROM user WHERE accno = ?";
        PreparedStatement psUser = conn.prepareStatement(userSql);
        psUser.setString(1, accno);
        ...
        errorMessage = "Error fetching account type: " + e.getMessage();
    }
    ...
    try {
        ...
        String balanceSql = "SELECT balance FROM balance WHERE accno = ?";
        PreparedStatement psBalance = conn.prepareStatement(balanceSql);
        psBalance.setString(1, accno);
        ResultSet rsBalance = psBalance.executeQuery();

        if (rsBalance.next()) {
            balance = rsBalance.getBigDecimal("balance");
        }
        ...
        errorMessage = "Error retrieving balance: " + e.getMessage();
    }
%>
<body class="g-sidenav-show  profilemain" style="overflow-x:hidden;">
    <aside
        ...
        <div class="collapse navbar-collapse  w-auto vh-100" id="sidenav-collapse-main">
            <ul class="navbar-nav">
				...
				<!-- Exchange Converter -->
		        <li class="nav-item">
		          <a class="nav-link " id="hover-link" href="exchange_converter.jsp">
		            <div class="icon icon-shape icon-sm shadow border-radius-md text-center me-2 d-flex align-items-center justify-content-center bg-purple">
		              <svg width="12px" height="12px" viewBox="0 0 43 36" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
		                <title>credit-card</title>
		                <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
		            ...
		            <span class="nav-link-text ms-1 text-white" style="font-size:13px;"><%=LanguageHelper.getMessage("exchange_converter", lang)%></span>
		          </a>
		        </li>
        ...
        <div class="container-fluid py-1 px-3">
          <nav aria-label="breadcrumb" style="padding-top:0px;">
                    <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 ps-2 me-sm-6 me-5">
                        ...
                        <li class="breadcrumb-item text-sm text-white active" aria-current="page"><%=LanguageHelper.getMessage("history", lang)%></li>
                    </ol>
...
<div class="w-100 position-relative" style="padding-left:2rem;">
    <div class="d-flex justify-content-between align-items-end" style="margin-left:10px; padding-bottom:30px;">
        <div class="d-flex align-items-center">
            
            <div>
            	<h6 class="mb-1 text-white font-weight-bolder">
            		<%=LanguageHelper.getMessage("acc_no", lang)%>: <%= session.getAttribute("accno") %>
            	</h6>
                <h6 class="mb-1 text-white font-weight-bolder">
                    <%=LanguageHelper.getMessage("acc_type", lang)%>: <%= acctype %> <!-- Injecting fetched account type here -->
                </h6>
        ...
        <div style="margin-left:10px; margin-bottom:20px;display: flex; align-items: center; gap: 30px;">
    <div style="display: flex; align-items: center; gap: 10px;">
        <select id="exportType" style="border: 1px solid black; border-radius:5px; padding: 8px; cursor: pointer;">
            <option value="pdf" style="background-color: white; color: black;" onmouseover="this.style.backgroundColor='#9b59b6'; this.style.color='white';" onmouseout="this.style.backgroundColor='white'; this.style.color='black';">PDF</option>
            <option value="doc" style="background-color: white; color: black;" onmouseover="this.style.backgroundColor='#9b59b6'; this.style.color='white';" onmouseout="this.style.backgroundColor='white'; this.style.color='black';">DOC</option>
            <option value="excel" style="background-color: white; color: black;" onmouseover="this.style.backgroundColor='#9b59b6'; this.style.color='white';" onmouseout="this.style.backgroundColor='white'; this.style.color='black';">Excel</option>
        </select>
        <button id="exportBtn" class="bg-glass3" 
            style="background-color: white; color: black; border: 1px solid black; border-radius:5px; padding: 8px 12px; cursor: pointer; transition: background 0.3s ease-in-out, border 0.3s ease-in-out;"
            onmouseover="this.style.background='linear-gradient(45deg, #ff6b6b, #f06595)'; this.style.color='white'; this.style.border='1px solid transparent';"
            onmouseout="this.style.background='white'; this.style.color='black'; this.style.border='1px solid black';">
            <%=LanguageHelper.getMessage("export", lang)%>
        </button>
    </div>

    <!-- Sorting Form -->
    <form method="get" style="display: flex; align-items: center; gap: 5px;">
        <label for="sortOrder" style="color:#fff; "><%=LanguageHelper.getMessage("sort", lang)%>: </label>
        <select name="sortOrder" id="sortOrder" onchange="this.form.submit()" style="color: black; border: 1px solid black; border-radius:5px; padding: 8px; cursor: pointer;">
            <option value="DESC" style="background-color: white; color: black;" <%= "DESC".equals(request.getParameter("sortOrder")) ? "selected" : "" %>>
            <%=LanguageHelper.getMessage("newest", lang)%></option>
            <option value="ASC" style="background-color: white; color: black;" <%= "ASC".equals(request.getParameter("sortOrder")) ? "selected" : "" %>>
            <%=LanguageHelper.getMessage("oldest", lang)%></option>
        </select>
    </form>

    <!-- Search Input and Button -->
    <div style="display: flex; align-items: center; gap: 5px;">
        <input type="text" id="searchInput" placeholder="Enter..." style="padding: 8px; border: 1px solid black; border-radius:5px; ">
        <button id="searchBtn" class="bg-glass3"
            style="background-color: white; color: black; border: 1px solid black; border-radius:5px; padding: 8px 12px; cursor: pointer; transition: background 0.3s ease-in-out, border 0.3s ease-in-out;"
            onmouseover="this.style.background='linear-gradient(45deg, #ff6b6b, #f06595)'; this.style.color='white'; this.style.border='1px solid transparent';"
            onmouseout="this.style.background='white'; this.style.color='black'; this.style.border='1px solid black';">
            Search
...
<script>
    document.getElementById('exportBtn').addEventListener('click', function() {
        ...
        const table = document.getElementById("example23");
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            for (let j = 0; j < rows[i].cells.length; j++) {
                rowData.push(rows[i].cells[j].innerText.trim());
            }
        ...
        if (exportType === 'pdf') {
            ...
            doc.text("History Table Export", 14, 10);
            
            doc.autoTable({
                head: [tableData[0]],  // Table headers
                body: tableData.slice(1), // Table data
                startY: 20,
                styles: { fontSize: 9 }, // Adjust text size
            });

            doc.save('History Table.pdf');

        } else if (exportType === 'doc') {
            let docContent = "<html><head><style>";
            docContent += "table { width: 100%; border-collapse: collapse; font-size: 12pt; }";
            docContent += "th, td { border: 1px solid black; padding: 5px; text-align: left; }";
            docContent += "</style></head><body>";
            docContent += "<h2>History Table Export</h2>";
            docContent += "<table>";
            ...
            let textBlob = new Blob([docContent], { type: "application/msword" });
            let link = document.createElement("a");
            link.href = URL.createObjectURL(textBlob);
            link.download = "History Table.doc";
            link.click();
            ...
            XLSX.writeFile(wb, 'History Table.xlsx');
        }
    });
    document.getElementById('searchBtn').addEventListener('click', function() {
        let input = document.getElementById('searchInput').value.toLowerCase();
        let table = document.getElementById("example23");
        ...
        <div class="container-fluid py-4 bg-glass3" style="margin-left:10px; width:920px;">
        <!-- Start Page Content -->
        <div class="row my-4">
        	<div class="col-lg-8 col-md-6 mb-md-0 mb-4"style="margin-right:5px; width:1050px;">
        	
                <div class="col-lg-12">
                
                		<div class="card-header pb-0 px-3 bg-transparent" style="margin-top:-10px;">
                            <div class="row">
                                <div class="col-md-6">
                                    <h6 class="mb-0 text-white"><%=LanguageHelper.getMessage("my_history", lang)%></h6>
                                </div>
                    ...
                    <div class="card_review card-outline-primary">
						<div class="summary-box" style="display: flex; justify-content: space-between; padding: 10px 15px;font-weight: bold;">
						    <p style="margin: 0;"><span style="color: #000000;font-weight: 700;"> <%=LanguageHelper.getMessage("income", lang)%>: <%= totalIncome %></span></p>
						    <p style="margin: 0;"><span style="color: #000000;font-weight: 700;"> <%=LanguageHelper.getMessage("outcome", lang)%>:<%= totalOutcome %></span></p>
						</div>
             
                        <div class="table-responsive m-t-40">
						  <table id="example23" class="display nowrap table table-hover text-center table-striped table-bordered" cellspacing="0" width="100%">
						    <thead class="thead-dark">
						        <tr>
						            <th><%=LanguageHelper.getMessage("id", lang)%></th> <!-- Transfer ID -->
						            <th><%=LanguageHelper.getMessage("sender_acc", lang)%></th>
						            <th><%=LanguageHelper.getMessage("receiver_acc", lang)%></th>
						            <th><%=LanguageHelper.getMessage("amt", lang)%></th>
						            <th><%=LanguageHelper.getMessage("transfer_type", lang)%></th>
						            <th><%=LanguageHelper.getMessage("transfer_time", lang)%></th>
						            <th><%=LanguageHelper.getMessage("acc_type", lang)%></th>
						            <th><%=LanguageHelper.getMessage("note", lang)%></th>
						        </tr>
						    </thead>
						    <tbody>
        <%
            // If there was no error, process the ResultSet
        ...
        <tr>
            ...
            <td><%= rs.getTimestamp("transfer_time") %></td>
            <td><%= rs.getString("account_type") %></td>
            <td><%= rs.getString("note") != null ? rs.getString("note") : "N/A" %></td>
        </tr>
        <%
                    }
                } catch (SQLException ex) {
                    errorMessage = "Error processing ResultSet: " + ex.getMessage();
                }
            }
        %>
    </tbody>
	  ...
	  <div class="modal-dialog">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title" id="frozenModalLabel">Account Frozen</h5>
	        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
	      </div>
	      <div class="modal-body">
	        Your account is currently freeze. Please contact support.
	      </div>
